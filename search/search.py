...
def depthFirstSearch(problem: SearchProblem) -> List[Directions]:
    ...
    while not fringe.isEmpty():
        ...
        next_state = problem.getSuccessors(state)
...
def uniformCostSearch(problem: SearchProblem) -> List[Directions]:
    ...
    while not fringe.isEmpty():
        ...
        for next in next_state:
            if next[0] not in explored:
                fringe.push((next[0], path + [next[1]]), problem.getCostOfActions(path + [next[1]]))
    util.raiseNotDefined()
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not fringe.isEmpty():
        ...
        if state not in explored or cost < best_g.get(state, float('inf')):
            explored.append(state)
            best_g[state] = cost
            ...
            for next_state, action, step_cost in problem.getSuccessors(state):
                ...
                if next_state not in explored or new_cost < best_g.get(next_state, float('inf')):
                    fringe.push((next_state, path + [action], new_cost), priority)
    util.raiseNotDefined()
